...
import static com.example.ecookbook.PrintSteps.sutlacSteps;

import com.example.ecookbook.TreeNode;


public class sutlacController {

    @FXML
    private TextArea sutlacText;
    @FXML
    private TextArea sutlacIngredientText;

    @FXML
    void sutlacRecipe(MouseEvent event) {

        sutlacText.appendText(sutlacSteps());
    }

    @FXML
    void sutlacIngredients(MouseEvent event) {
        AVLTree sutlacTree = new AVLTree();
        sutlacTree.root = sutlacTree.insert(sutlacTree.root, "milk");
        sutlacTree.root = sutlacTree.insert(sutlacTree.root, "rice");
        sutlacTree.root = sutlacTree.insert(sutlacTree.root, "water");
        sutlacTree.root = sutlacTree.insert(sutlacTree.root, "rice flour");
        sutlacTree.root = sutlacTree.insert(sutlacTree.root, "granulated sugar");
        sutlacTree.root = sutlacTree.insert(sutlacTree.root, "cinnamon");

        String sutlacIngredients = sutlacTree.printIngredients();

        sutlacIngredientText.appendText(sutlacIngredients);
    }
